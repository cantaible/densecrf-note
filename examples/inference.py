...
# <<是左移
# shape:(240,320)
# [0,128,64]-->4227072=2^16*64+2^8*128

# Convert the 32bit integer color to 1, 2, ... labels.
# Note that all-black, i.e. the value 0 for background will stay 0.
colors, labels = np.unique(anno_lbl, return_inverse=True)
# 返回annotation中颜色值colors并按照第一次出现顺序赋值索引号labels
# But remove the all-0 black, that won't exist in the MAP!
HAS_UNK = 0 in colors
if HAS_UNK:
    # HAS_UNK：若annotation中黑色像素指代unknown
...
# 取colors二进制的第9位到16位，并除以2^8
colorize[:,2] = (colors & 0xFF0000) >> 16
# 把colors还原回去
# [[0,64,0],[0,128,64]]原图中只有这两种rgb颜色
# Compute the number of classes in the label image.
# We subtract one because the number shouldn't include the value 0 which stands
# for "unknown" or "unsure".
n_labels = len(set(labels.flat)) - int(HAS_UNK)
# set：获取无序不重复元素集，flat：将ndarray展开成1-D可迭代
...
if use_2d:
    ...
    # get unary potentials (neg log probability)
    U = unary_from_labels(labels, n_labels, gt_prob=0.7, zero_unsure=HAS_UNK)
    # U.shape=(2,76800),76800个像素，每个像素有两个值0,1,分别对应了非unsure的两个标签
    # 若某像素n的label=m，则U[m,n]=p_energy,该列其他值=n_energy
    # 若某像素n的label=unsure。则U[:,n]=-np.log(1.0 / n_labels)
    d.setUnaryEnergy(U)

    # This creates the color-independent features and then add them to the CRF
    # 创建颜色独立的特征并添加进CRF
    feats = create_pairwise_gaussian(sdims=(3, 3), shape=img.shape[:2])
    # feats.shape=(2,76800),每一行是meshgrid的展开
